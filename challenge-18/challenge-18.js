(function() {
  ...
  */
  ...
  function cleanCPF(cpf) {
    ...
    //substituindo tudo que não é número \D por ''
  }
  ...
  */
  ...
  cpfs.forEach(function(cpf){
    ...
    //cada captura (\d\d\d) que é o que está dentro dos parenteses
    //replace em cima de cada captura acrescido deo sinal de . ou -
    //cada $ representa um captura
  });
  //podemos fazer de outra forma utilizando função no lugar de '$1.$2.$3-$4':
  console.log('***************************');
  cpfs.forEach(function(cpf){
  ...
  */
  ...
  console.log('Os meses de janeiro, junho e julho começam com a letra j.'.match(
    // podemos reescrever --> /junho|julho/g desta forma otimizada:
    /ju[nl]ho/g   //match com ju seguido de n ou l seguido de ho (trará junho e julho)
  ) );
  ...
  */
  ...
  console.log('<div><ul><li></li><li></li><li><span></span></li></ul></div>'
    .match(/<\w+><\/\w+>/g)   //tag de abertura <\w+> tag de fechamento <\/\w+>
  );
  ...
  */
  ...
  // aula 112
  // p1 match1 titulo da pg match2 paragrafo match3 Rodapé, vide abaixo
  // /<\w+>([^<]+)<\/\w+>/g
  // agora pegando as tags
  // /(<\w+)>([^<]+)<\/\w+>/g   nome da tag (<\w+) e texto da tag ([^<]+)
  // fechamento da tag <\/\w+>
  console.log(
    '<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>'
    .replace(
